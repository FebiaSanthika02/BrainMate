    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', color: 'var(--text-main)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px', paddingBottom: '10px', borderBottom: '1px solid var(--border-light)' }}>
        <h3 style={{ fontSize: '18px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Sparkles color="var(--color-pink)" size={20} />
          Smart Summary
        </h3>
        {!loading && !error && (
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button style={{ padding: '8px 16px', background: 'var(--color-pink)', color: 'white', borderRadius: '8px', border: 'none', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}>
              Export PDF
            </button>
          </div>
        )}
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '200px', gap: '15px' }}>
            <Loader2 className="animate-spin" size={40} color="var(--color-pink)" style={{ animation: 'spin 1s linear infinite' }} />
            <p style={{ fontSize: '15px', fontWeight: 600, color: 'var(--text-muted)' }}>AI is analyzing and summarizing the material...</p>
          </div>
        ) : error ? (
